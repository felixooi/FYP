...
def load_data(filepath=None, s3_bucket=None, s3_key=None):
    ...
    if filepath:
        ...
        source = filepath
        print(f"[OK] Dataset loaded from local: {source}")
        ...
        print(f"[OK] Dataset loaded from S3: {source}")
    else:
...
def inspect_data(df):
    """Comprehensive data inspection for notebook use."""
    ...
    num_summary = df.describe().T.round(2).reset_index().rename(columns={"index": "Feature"})
